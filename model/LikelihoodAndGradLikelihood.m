function [likelihood, grad_likelihood] = ...
    LikelihoodAndGradLikelihood(X_collective, W_params, datatypes, n_cols, M_collective)
...
M_sources = {M_collective(:, 1:n_cols(1))};%%%%
...
while k <= size(n_cols, 2)-1
    ...
    M_sources{end+1} = M_collective(:, n_cols(k)+1:n_cols(k) + n_cols(k+1));%%%%%
    k = k + 1;
end
...
while v <= size(n_cols, 2)
    ...
    likelihood = M_sources{v}.*(X_sources{v} .* W_sources{v} - temp_mat);
    likelihood = sum(likelihood(:));
    likelihoods(end+1) = likelihood;

    % grad
    temp_mat_prime = link_function_prime(W_sources{v});
    grad =  M_sources{v}.*(X_sources{v} - temp_mat_prime);
    grads{end+1} = grad;
    v = v + 1;
end 
